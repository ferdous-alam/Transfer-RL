...
def interact(env, steps=10000, max_ep_len=500, 
    ...
    if save_data == True:
        json.dump(dataset, codecs.open('{}.json'.format(dataset_name), 'w', encoding='utf-8'),
                separators=(',', ':'),
                sort_keys=True,
                indent=4)
...
def create_dataset(env_fn, train_steps, 
    ...
    # create dataset
    interact(env, steps=train_steps, dataset_name=train_dataset_name)
    interact(env, steps=eval_steps, dataset_name=eval_dataset_name)
...
def train_model(train_data, 
    ...
    while True:
        ...
        if early_stopper.early_stop(latest_validation_loss): 
            break
    if save:
        torch.save(model.state_dict(), '/media/ghost-083/SolarSystem1/1_Research/00_Transfer-RL/data/ant/task_sim_dataset/{}_{}.pth'.format(test_name, model_name))
...
def learn_models(env_fn, train_path, eval_path, test_name): 
    ...
    # model 
    dyn_model = DynamicsModel(in_features=in_features, out_features=out_dyn_features)
    rew_model = RewardModel(in_features=in_features, out_features=out_rew_features)
...
if __name__ == "__main__":
    # env_fn = lambda: gym.make("Pendulum-v1", g=18.0)
    ...
    # ---- create dataset ----
    # create_dataset(env_fn, train_steps=100000, 
    #                     eval_steps=20000, 
    #                     train_dataset_name=train_data_name, 
    #                     eval_dataset_name=eval_data_name)

    # ---- fit model ----
    # save_path = '/media/ghost-083/SolarSystem1/1_Research/00_Transfer-RL/data/'
    # d_path = '/home/ghost-083/Research/1_Transfer_RL/Task_similarity/env_mods/'
    # xml_path = d_path + 'half-cheetah/assets/half_cheetah_0.xml'

    for k in range(1, 5, 1):
        print(f'xml --------------> {k}')
        save_path = '/media/ghost-083/SolarSystem1/1_Research/00_Transfer-RL/data/'
        train_data_name =  save_path + f'ant/task_sim_dataset/ant_train_xml{k}'
        eval_data_name = save_path + f'ant/task_sim_dataset/ant_eval_xml{k}'

        d_path = '/home/ghost-083/Research/1_Transfer_RL/Task_similarity/env_mods/Ant/assets/'
        xml_path = d_path + f'ant_{k}.xml'
        env_fn = lambda: gym.make('Ant-v3', xml_file=xml_path)   
        train_path = save_path + f'ant/task_sim_dataset/ant_train_xml{k}.json'
        eval_path = save_path + f'ant/task_sim_dataset/ant_eval_xml{k}.json'
        learn_models(env_fn, train_path, eval_path, f'ant_xml{k}')
