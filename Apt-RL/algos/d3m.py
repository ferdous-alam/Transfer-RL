...
from algos.sac_utils import relabel_batch
from algos.model_train_utils import update_models
import logging 



def run_d3m(env_fn, total_steps, lr=1e-3, learning_starts=2000, update_every=50, 
  ...
  model_buffer = ReplayBuffer(obs_dim=obs_dim, act_dim=act_dim, size=replay_buffer_size)
  ...
  for t in range(total_steps):
    ...
    if t >= learning_starts and t % update_every == 0:
      ...
      #TODO: update model  
      dyn_model, rew_model = update_models(env, replay_buffer, lr=1e-3)
      ...
      for j in range(num_grad_updates):
        ...
        #TODO: relabel batch
        batch = relabel_batch(batch, dyn_model, rew_model)
