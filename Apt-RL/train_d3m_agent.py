import gym
from algos import apt, apt_fixed_source, d3m
import time
import argparse
...
def run_pendulum():
    ...
    source_path = 'data/pendulum/pendulum_source_optimal/policy.pth'
    ...
    update_every = 100                                       # fixed for all tasks
    ...
    num_grad_updates = 100                                   # fixed for all tasks
    num_test_episodes = 10                                  # fixed for all tasks
    beta = 100.0                                            # fixed for all tasks


    # run algo 
    test_name = f'pendulum_apt_g_{g}_d3m'
    start = time.time()
    env_fn = lambda: gym.make("Pendulum-v1", g=g)
    # train agent 
    d3m.run_d3m(env_fn, total_steps=total_steps, lr=lr, learning_starts=learning_starts, 
...
def run_ant():
    ...
    xmls = [] 
    for k in range(5):
        xml = f'/home/ghost-083/Research/1_Transfer_RL/D3M/envs/Ant/assets/ant_{k}.xml'
        xmls.append(xml)
